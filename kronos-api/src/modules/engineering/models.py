"""
Modelos de Ingeniería - KronosSystem.
Establece las relaciones necesarias para cálculos industriales en Producción y Logística.
"""
...
class FTVersion(Base):
    ...
    # RELACIONES CRÍTICAS PARA CÁLCULOS
    ficha = relationship("FichaTecnica", back_populates="versiones")
    sustrato = relationship("src.modules.common.models.Sustrato") # <-- Corregido
    juego_cilindro = relationship("JuegoCilindro")
    cirel = relationship("Cirel")
    
    config_empaque = relationship("FTConfigEmpaque", back_populates="version", uselist=False)
    ordenes = relationship("src.modules.production.models.OrdenCompra", back_populates="version_ft")
